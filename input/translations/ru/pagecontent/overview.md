...
* Строка 10 добавляет расширение к профилю, используя местное расширение `EvidenceType`, дает ему локальное имя `evidenceType` и назначает кардинальность 0..*. _EvidenceType определен в строке 31._
...
* Строки 13-20 ограничивают кардинальность некоторых унаследованных элементов. FSH не поддерживает установку кардинальности нескольких элементов одновременно, поэтому это должны быть отдельные строки.
...
* Один файл на элемент FSH (например, профиль или набор значений)
...
* Создание папок для каждого типа элемента (профили, расширения, наборы значений) с отдельными файлами для каждого элемента соответствующего типа внутри этих папок.
...
Когда SUSHI запускается (3), он собирает все файлы FSH из папки **input/fsh** (1) и записывает сгенерированные артефакты FHIR JSON в данную папку (4). Входные и выходные папки появляются как аргументы командной строки SUSHI. Если не указаны, входная папка по умолчанию будет текущей папкой, а выходы будут записаны в **./fsh-generated**.
